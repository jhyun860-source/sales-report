...
export default function Home() {
  ...
  useEffect(() => {
    if (!authLoading && user && isStaffLiquorOnly(user.loginId)) {
      ...
      navigate(`/liquor-stock?date=${getTodayString()}${branchId ? `&branchId=${branchId}` : ''}`);
    }
  ...
  // 관리자: URL branchId → localStorage 선택값 → 첫 지점 순서로 유지
  // 매니저/직원: localStorage에 남은 다른 지점값을 무시하고 계정 지점으로 고정
  const selectedBranchId: number | null = (() => {
    ...
    if (urlBranchId && myBranches.some(b => b.id === urlBranchId)) return urlBranchId;
  ...
  // 가장 최근 유효 마감금을 화면 입력값에도 즉시 채워준다.
  useEffect(() => {
    if (!autoCalculatedPosStartAmount || autoCalculatedPosStartAmount <= 0) return;
    setRecord(prev => {
      const currentStart = parseAmount(prev.posStartAmount);
      if (currentStart > 0) return prev;
      return { ...prev, posStartAmount: String(autoCalculatedPosStartAmount) };
    });
  }, [currentDate, selectedBranchId, autoCalculatedPosStartAmount]);
  ...
  return (
    <div className="min-h-screen" style={{ background: 'oklch(0.985 0.008 85)' }}>
      ...
      >
        ...
        {/* 2줄: 액션 버튼들 */}
        <div className="flex flex-wrap items-center gap-1.5 px-4 pb-2.5 overflow-visible">
          {/* 알림 토글 */}
          {isSupported && (
            <button
          ...
          {/* 주류 출고현황 버튼 */}
          <button
            onClick={() => navigate(`/liquor-stock?date=${getTodayString()}${selectedBranchId ? `&branchId=${selectedBranchId}` : ''}`)}
            className="flex items-center gap-1 px-2.5 h-8 sm:px-3 sm:h-9 rounded-lg text-xs sm:text-sm font-medium border transition-colors flex-shrink-0"
            style={{
              background: 'oklch(0.92 0.015 85)',
              color: 'oklch(0.25 0.01 50)',
