...
## 출근자 인센티브-기타사항 사이 아르바이트 시간 + 추가판매 총계 섹션 추가

- [x] 현재 TableReport.tsx 출근자 인센티브/기타사항 렌더링 구조 확인
- [x] 서버 수정 불필요 (인센티브 데이터가 이미 로컈 state에 있음)
- [x] 프론트엔드: 아르바이트 총 근무시간 표시 (이름별 + 합계)
- [x] 프론트엔드: 윥4추가/병추가/맥주병추가 총계 표시 (직원+아르바이트 합산)

## 형광펜 X버튼 개별 제거 버그 수정

- [x] MemoEditor.tsx: 각 mark 태그 안에 × 버튼 주입 (MutationObserver로 동적 추가)
...
- [x] 1일 리셋 로직 수정 - 현재 2일부터 리셋되는 문제 해결 (날짜 비교 로직 개선)
- [x] 관리자 대시보드에 "누적금액 수동 리셋" 버튼 추가
- [x] 수동 리셋 시 확인 다이얼로그 표시 (실수 방지)
- [x] 리셋 후 성공 메시지 표시
...
## 긴급 작업 - 빨간창 오류 제거, 시제 넘김 문제, 외부 유지보수 자료 준비

- [x] 빨간창 오류 제거 - Home.tsx에서 매출 불일치 경고 완전 제거 (날짜 넘김 시 반복되는 버그 해결)
- [x] 시제 넘김 문제 전지점 수정 - 모든 지점에서 날짜 넘김 시 시작금이 정상 이월되도록 확인 (autoCalculatedPosStartAmount로 해결)
- [x] 외부 유지보수 자료 준비 - 소스코드 zip, package.json, DB 스키마, 환경변수, 배포 가이드, 데이터 백업

## UI 레이아웃 및 제품 설정 패치 (salesdash-liquor-layout-product-settings-home-header-v1)

- [x] 입고/출고 화면의 날짜·위치·제품·메모 줄맞춤 정리
- [x] 메모 글자 깨짐/세로 잘림 문제 수정
- [x] 제품 상세 화면에서 톱니바퀴/제품 설정 버튼 클릭 시 수정창 표시
- [x] 제품명, 카테고리, 현재 수량, 단가 수정 가능하게 연결
- [x] 매출보고 화면 저장/로그아웃 버튼을 상단 지점명 옆으로 이동
- [x] 기존 주류 재고/입고/출고/히스토리 기능 유지
- [x] pnpm build 성공 (오류 없음)
- [x] vitest 42개 테스트 전체 통과

## 장바구니 삭제 및 히스토리 수정/삭제 기능 (liquor-cart-history-edit-delete-v2)

- [x] 입고/출고/조정 장바구니에서 담은 품목 개별 삭제 X 버튼 추가
- [x] 장바구니 품목 - / + 버튼뿐 아니라 삭제 버튼으로 바로 제거 가능
- [x] 히스토리 내역별 수정 버튼 추가
- [x] 히스토리 수정 시 날짜, 수량, 메모 수정 가능
- [x] 히스토리 삭제 버튼 추가
- [x] 히스토리 삭제 시 해당 입고/출고/조정 수량만큼 재고 복구
- [x] 히스토리 수정 시 수량 변경분만큼 재고 자동 보정
- [x] 지점 계정은 자기 지점 히스토리만 수정/삭제 가능
- [x] 관리자 계정은 조회 가능한 지점 히스토리 수정/삭제 가능
- [x] 기존 주류 재고/제품관리/입고/출고/조정 기능 유지
- [x] pnpm build 성공 (오류 없음)
- [x] vitest 42개 테스트 전체 통과

## 히스토리 리스트/필터/원가 기능 (liquor-history-list-filter-cost-final-v2)

- [x] 히스토리 기본으로 날짜별 쭉 나오게 수정 (검색 필요 없음)
- [x] 날짜별 히스토리 카드 클릭 시 그날 입고/출고/조정 상세 내역 표시
- [x] 필터 화면 추가 (거래 종류, 기간, 제품명 검색)
- [x] 필터 화면에서 거래처/위치/멤버 항목 제거
- [x] 관리자 화면 히스토리 상세 클릭 시 출고 원가 0원 문제 수정
- [x] 출고 원가를 해당 날짜 상세내역에서 품목별 원가 합산으로 표시
- [x] 지점 계정은 자기 지점 히스토리만 표시 유지
- [x] 관리자 계정은 전체/지점별 조회 유지
- [x] pnpm build 성공 (오류 없음)
- [x] vitest 42개 테스트 전체 통과

## 주류 단가 동기화 패치 (liquor-unit-cost-sync-fix-v1)

- [x] 주류 기본 단가표 전체 재반영
- [x] 기존 DB에 이미 0원으로 생성된 품목도 단가표 기준으로 자동 보정
- [x] 칼라일, 캔터키, 코쿤위스키, 모엣샹동 매그넘, 야마자키, 아줄 레포사도, 론디아즈 등 이름 차이 있는 품목도 매칭
- [x] 맥켈란 18y = 450,000원
- [x] 아르망디 = 650,000원
- [x] 기네스 = 4,600원
- [x] 샴페인 카테고리 유지
...
- [x] vitest 42개 테스트 전체 통과
- [x] 기존 재고 수량 무손상 (단가/카테고리만 보정)

## 주류 단가 및 제품 삭제 기능 패치 (liquor-cost-delete-fix-final-v2)

- [x] 주류 단가표 전체 재반영
- [x] 기존 DB에 이미 0원으로 들어간 제품도 단가표 기준으로 자동 보정
- [x] 칼라일, 캔터키, 코쿤위스키, 론디아즈, 모엣샹동 매그넘, 야마자키, 아줄 레포사도 등 이름 차이 있는 품목도 매칭
- [x] 맥켈란 18y = 450,000원
- [x] 아르망디 = 650,000원
- [x] 기네스 = 4,600원
- [x] 제품 삭제 안 되는 문제 수정 (관리자: 전체 비활성화, 지점: 해당 지점 목록에서 숨김)
- [x] 지점에서 숨김 처리한 제품이 다시 제품 목록에 보이지 않도록 조회 로직 보강
- [x] pnpm build 성공 (오류 없음)
- [x] TypeScript 타입 체크 통과
- [x] vitest 42개 테스트 전체 통과
...
## 제품 즉시 삭제 기능 패치 (liquor-delete-immediate-fix-v3)

- [x] 제품 삭제 시 서버 응답/업데이트를 기다리지 않고 현재 화면 리스트에서 즉시 제거
- [x] 관리자도 삭제한 제품이 다시 보이지 않도록 includeInactive=false로 조회
- [x] 관리자 삭제는 제품 전체 비활성화 처리
- [x] 지점 계정 삭제는 해당 지점에서만 숨김 처리
- [x] 지점 숨김 처리된 제품은 해당 지점 제품 목록에서 다시 안 보이게 조회 로직 보강
- [x] 삭제 실패 시에는 다시 목록에 복구되도록 처리
- [x] 삭제 시 해당 제품이 장바구니에 있으면 장바구니에서도 제거
- [x] pnpm build 성공 (오류 없음)
- [x] TypeScript 타입 체크 통과
- [x] vitest 42개 테스트 전체 통과
- [x] DB 데이터 무손상 (소스코드만 교체)

## 관리자 지점 선택 유지 및 히스토리 배치 수정/삭제 기능 (liquor-admin-branch-history-batch-fix-v2)

- [x] 관리자 모드에서 지점 선택값을 localStorage에 저장해서 화면 이동/수정/뒤로가기 후에도 같은 지점 유지
- [x] 히스토리 상세 화면에서 전체 기록 날짜 수정 기능 추가 (그날 출고/입고/조정 묶음 전체 날짜 한 번에 변경)
- [x] 히스토리 상세 화면에서 전체 삭제 기능 추가 (해당 상세 내역 전체 삭제, 재고 복구)
- [x] 기존 개별 품목 수정/삭제 기능 유지
- [x] 지점 계정은 자기 지점 히스토리만 수정/삭제 가능
- [x] 관리자 계정은 조회 가능한 지점 히스토리 수정/삭제 가능
- [x] pnpm build 성공 (오류 없음)
- [x] TypeScript 타입 체크 통과
...
## 히스토리 배치 삭제 기능 (liquor-history-batch-delete-final-v3)

- [x] 히스토리 상세 화면에 "전체 삭제" 버튼 추가
- [x] 특정 날짜/묶음에 등록된 입고/출고/조정 내역 전체를 한 번에 삭제 가능
- [x] 전체 삭제 시 해당 묶음의 모든 품목 재고를 원래대로 복구 (출고/입고/조정 모두)
- [x] 기존 개별 품목 수정/삭제 기능 유지
- [x] 지점 계정은 자기 지점 히스토리만 전체 삭제 가능
- [x] 관리자 계정은 조회 가능한 지점 히스토리 전체 삭제 가능
- [x] pnpm build 성공 (오류 없음)
- [x] TypeScript 타입 체크 통과
- [x] vitest 42개 테스트 전체 통과
- [x] DB 데이터 무손상 (소스코드만 교체)
...
- [x] DB 계정 추가: s3, s4, d2, m3, m4 (비밀번호 1234) - SQL 스크립트 제공
- [x] DB 기존 계정 비밀번호 통일: m1, m2, d1, s1, s2 (비밀번호 1234) - SQL 스크립트 제공
- [x] 각 계정 로그인 테스트 및 메뉴 확인 - seed.mjs 수정 및 실행 완료
