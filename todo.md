...
## POS 마감금 → 다음 날 시작금 자동 반영

- [x] 이전 날짜 POS 마감금을 다음 날짜 POS 시작금으로 자동 반영 (새 날짜 이동 시 빈 시작금이면 이전 날 마감금으로 채움)

## POS 시작금 자동 반영 버그 수정

- [x] 4월 3일→4일 이동 시 마감금이 시작금으로 채워지지 않는 문제 수정
- [x] 근무 없는 날(4월 5일) 건너뛰어도 4월 6일 시작금에 마지막 마감금 반영되도록 수정
- [x] serverRecord가 undefined(로딩 중)일 때 자동 반영 useEffect 실행 방지

## 영업 안 한 날 건너뛰기 시 누적금/POS 미반영 버그

- [x] 일요일(영업 없음) 건너뛰고 월요일 이동 시 누적금 0으로 표시되는 문제 수정
- [x] 일요일 건너뛰고 월요일 이동 시 POS 시작금/마감금 미반영 문제 수정

## POS 시작금/마감금 연속 보장 및 매달 1일 누적 리셋

- [x] 4월 4일 posEndAmount 627000으로 보정 및 이후 날짜 posStartAmount 연쇄 수정
- [x] 서버 저장 시 매달 1일이면 누적금 리셋(당일 매출만), 나머지는 이전 누적 기준으로 계산
- [x] 서버 저장 시 posStartAmount가 0이면 이전 기록의 posEndAmount로 자동 채움
- [x] 클라이언트: 매달 1일이면 누적금 0부터 시작 (서버와 동일한 규칙 적용)

## 일요일 기록 일괄 보정 (전 지점)

- [x] 모든 지점의 일요일 기록에 "판매 0, 시작금=마감금=전날 마감금" 규칙 적용
- [x] 일요일 이후 날짜들의 누적금/POS 연쇄 재계산

## 전 지점 규칙 영구 반영

- [x] 서버 save mutation에 일요일 규칙 영구 반영 (일요일 저장 시 cash/card=0 강제, posStart=posEnd=prevPosEnd)
- [x] 모든 지점 기존 데이터 일괄 보정 (누적금 재계산, POS 연속, 일요일 규칙)

## POS 마감금→시작금 자동 반영 재수정

- [x] 전날 POS 마감금이 다음 날 시작금으로 넘어오지 않는 문제 완전 수정

## POS 시작금 자동 반영 재수정 (4월 6일→7일)

- [x] 4월 6일→7일 이동 시 POS 시작금이 전날 마감금으로 반영되지 않는 문제 수정

## 대치점 4월 6일 마감금 보정

- [x] 대치점 4월 6일 posEndAmount 0 → 527,000 보정
- [x] 대치점 4월 7일 이후 posStartAmount 연쇄 수정

## POS 마감금 자동 계산 저장 버그

- [x] POS 마감금(posEndAmount)이 자동 계산값(시작금 - 지출 + 현금입금)으로 DB에 저장되지 않는 문제 수정
- [x] 대치점 4월 6일 이후 posEndAmount 재보정

## 누적금 계산 버그 (화요일 이후 월요일 기준 고정)

- [x] 화요일 이후 cashTotal/cardTotal이 월요일 값으로 고정되는 원인 파악
- [x] 서버 누적금 계산 로직 수정
- [x] 영향받은 지점 데이터 재보정 (대치점 4월 7일, 12일 cardTotal 24,718,000으로 보정)
