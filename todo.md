...
## 긴급 버그 수정 - 날짜 넘김, 매출 불일치, 리셋 기록 보존

- [ ] 날짜 넘김 시 시작금 0 오류 - 이전 날짜 마감금이 다음날 시작금으로 자동 이월되도록 수정
- [ ] 매출 불일치 경고 오류 - 동기화 로직 개선 (불필요한 경고 제거)
- [ ] 리셋 시 전달 기록 보존 - 1일 이전 데이터는 유지, 1일 이후만 리셋


## 긴급 버그 수정 완료

- [x] 날짜 넘김 시 시작금 0 오류 - Home.tsx에서 autoCalculatedPosStartAmount로 자동 채우기 구현
- [x] 매출 불일치 경고 오류 - isMismatch 조건 개선 (dailyTotal > 0 조건 추가)
- [x] 리셋 시 전달 기록 보존 - manualResetCumulativeAmounts에서 1일 이후 데이터만 리셋
