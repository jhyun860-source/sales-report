...
async function ensureCanonicalStoreAccounts() {
  ...
  for (const spec of CANONICAL_STORE_ACCOUNTS) {
    ...
    if (existing) {
      // IMPORTANT: Do not overwrite an existing account password during deploy/restart.
      // v1/admin에서 변경한 비밀번호는 운영 DB에 저장된 passwordHash가 진짜 값입니다.
      // 기본 계정 동기화는 지점/표시명/권한 같은 메타데이터만 보정하고,
      // passwordHash는 계정이 처음 생성될 때만 1234로 넣습니다.
  ...
  tableReport: router({
    ...
    batchSave: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // 2. 테이블 항목 배치 처리
        // INSERT는 insertId 충돌 방지를 위해 순차 처리, UPDATE는 병렬 처리
    ...
    analyzeOrderMemo: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const yellowGuide = yellowKeywords.length > 0
          ? `노란 형광펜(<mark style="background: rgb(255, 224, 102); border-radius: 2px; padding: 0px 1px;">텍스트</mark>): 주류/샴페인/위스키/맥주 등을 "병" 단위로 주문한 경우에만 적용(양주는 바틀, 맥주도 병이면 포함). 잔술/글라스/생맥주(드래프트 잔) 단위 주문은 절대 노란 형광펜 금지. 이전 기록에서 노란 형광펜이 적용된 키워드 예시: ${yellowKeywords.join(', ')}`
          : '노란 형광펜: 주류/샴페인/위스키/맥주를 "병" 단위로 주문한 경우에만 적용 (잔술/글라스/생맥주잔 단위는 절대 금지)';
...
1. items: 이미지의 각 줄(항목)을 하나씩 빠짐없이 배열로 반환
   ...
   - highlight: 이 항목에 적용할 형광펜 색상
     · "yellow": 주류/샴페인/위스키/맥주 등을 "병" 단위로 주문한 경우만 (예: 기네스 드래프트/카스/테라 등 맥주도 병 단위면 포함) (⚠️ 잔술/글라스/잔/생맥주잔 단위는 절대 yellow 금지 → "none")
